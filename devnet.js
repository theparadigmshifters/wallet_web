const CONFIG={API_URL:'https://eon.zk524.com/dev',FAUCET_URL:'https://eon.zk524.com/dev',TURNSTILE_SITE_KEY:'0x4AAAAAACvuWReT5CoqVcfm',REFRESH_INTERVAL:10000};
const STATE={currentPage:'wallet',currentWalletId:null,wallets:{},refreshTimer:null};
function loadWallets(){
const stored=localStorage.getItem('eon_devnet_wallets');
const currentId=localStorage.getItem('eon_devnet_current_wallet');
if(stored){
...
localStorage.setItem('eon_devnet_wallets',JSON.stringify(STATE.wallets));
if(STATE.currentWalletId){
localStorage.setItem('eon_devnet_current_wallet',STATE.currentWalletId);
}
}
...
localStorage.setItem('eon_devnet_current_wallet',id);
...
if(widget){widget.style.display='none';const lbl=widget.parentElement;if(lbl)lbl.style.display='none';}
...
document.getElementById('currentEndpoint').textContent=CONFIG.API_URL;
